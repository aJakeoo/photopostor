...
const WORD_BANK = [
  ...
  { word: 'Floppers',    category: 'NBA Players' },
];
...
function renderShowcase(r) {
  ...
  const g = $('gallery');
  g.innerHTML = '';
  // Seeded shuffle so every device shows the SAME order, but the order
  // is fresh each round — no player is locked into "explains first" by
  // virtue of joining the lobby earliest.
...
function showSpectator() {
  ...
  // Use the same seeded shuffle as the main gallery so the spectator's
  // view matches what active players are looking at.
  const subs = r.submissions || {};
